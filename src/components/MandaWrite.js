...
async function fetchMandaData(mainId) {
  ...
  try {
    const response = await axios.get(`http://127.0.0.1:8000/manda/mandasimple/${mainId}`);
  ...
  useEffect(() => {
    ...
    const saveData = async () => {
      try {
        ...
        // 서버로 데이터 전송
        const response = await axios.post('http://127.0.0.1:8000/manda/edit/content/', requestData);
